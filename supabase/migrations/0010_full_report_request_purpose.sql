alter table leads add column if not exists request_purpose text;

alter table leads drop constraint if exists leads_check;
alter table leads add constraint leads_full_report_contact_check check (
  request_purpose is null or (
    preferred_contact_method = 'phone'
    and phone is not null
    and email is not null
  )
) not valid;
alter table leads add constraint leads_request_purpose_check check (
...
drop function public.submit_lead_once(uuid, integer, jsonb, uuid, jsonb, uuid, text, text, text, text, text, text, text, timestamptz);
...
create function public.submit_lead_once(
  ...
  p_line_id text, p_request_purpose text, p_consent_version text,
...
begin
  ...
  if p_phone is null or p_email is null then raise exception 'FULL_REPORT_CONTACT_REQUIRED' using errcode = 'P0001'; end if;
  if p_request_purpose not in ('view_full_report', 'planning_to_build', 'compare_options', 'architect_consultation', 'budget_planning', 'design_service', 'other') then raise exception 'INVALID_REQUEST_PURPOSE' using errcode = 'P0001'; end if;
  ...
  if coalesce(jsonb_typeof(p_snapshot), '') <> 'object' or coalesce(jsonb_typeof(p_snapshot->'estimate'), '') <> 'object' then
    raise exception 'INVALID_CALCULATION_SNAPSHOT' using errcode = 'P0001';
  end if;
  normalized_snapshot := jsonb_set(
    jsonb_set(
      jsonb_set(
        jsonb_set(p_snapshot, '{pricingVersion}', to_jsonb(published_version), true),
        '{referenceDate}', to_jsonb(to_char(published_reference_date, 'YYYY-MM-DD')), true
      ),
      '{estimate,pricingVersion}', to_jsonb(published_version), true
    ),
    '{estimate,referenceDate}', to_jsonb(to_char(published_reference_date, 'YYYY-MM-DD')), true
  );
  ...
  insert into leads (configuration_id, idempotency_key, name, preferred_contact_method, phone, email, line_id, request_purpose, consent_version)
    values (p_configuration_id, p_idempotency_key, p_name, p_preferred_contact_method, p_phone, p_email, p_line_id, p_request_purpose, p_consent_version) returning id into existing_lead;
...
revoke all on function public.submit_lead_once(uuid, integer, jsonb, uuid, jsonb, uuid, text, text, text, text, text, text, text, text, timestamptz) from public, anon, authenticated;
grant execute on function public.submit_lead_once(uuid, integer, jsonb, uuid, jsonb, uuid, text, text, text, text, text, text, text, text, timestamptz) to service_role;
