...
vi.mock("./full-report", () => ({ FullReport: ({ pdfHref, onRequestConsultation, onConsultationRequested }: { pdfHref: string; onRequestConsultation: (signal: AbortSignal) => Promise<void>; onConsultationRequested: () => void }) => <><a href={pdfHref}>pdf</a><button onClick={() => { void onRequestConsultation(new AbortController().signal).then(onConsultationRequested).catch(() => undefined); }}>consult</button></> }));
