...
import type { EstimateMode, PriceBook } from "../domain/price-book";
import { toCalculationConfiguration, type EstimateRequest } from "./estimate-request";

export type PriceBookRepository = {
  loadPublished(): Promise<{ priceBookId: string; priceBook: PriceBook; areaCatalog: AreaCatalog; estimateMode?: EstimateMode }>;
};
...
export async function estimateProject(
  ...
  const { priceBook, areaCatalog, estimateMode = "published" } = await priceBookRepository.loadPublished();
  const area = calculateArea(configuration, areaCatalog);
  const estimate = calculateEstimate({
    configuration,
    constructionFloorAreaM2: area.constructionFloorAreaM2,
    production: estimateMode === "published",
  }, priceBook);

  return buildFreePreview(configuration, area, estimate, estimateMode);
}
