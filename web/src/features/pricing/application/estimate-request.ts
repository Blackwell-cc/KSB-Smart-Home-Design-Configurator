...
} from "@/features/configurator/domain/material-catalog";
...
const MATERIAL_QUALITY_FOR_LEVEL = {
  select: "standard",
  premium: "premium",
  signature: "signature",
} as const satisfies Record<EstimateRequest["materialLevel"], HouseConfiguration["materialQualityId"]>;
...
export function projectEstimateRequest(configuration: HouseConfiguration): EstimateRequest {
  ...
  const request = EstimateRequestSchema.safeParse({
    ...
    siteAccess: configuration.siteAccess, materialLevel, specialFeatures,
  });
