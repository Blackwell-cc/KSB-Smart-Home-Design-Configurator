...
export function ReviewStep({ configuration, onEdit }: { configuration: HouseConfiguration; onEdit(step: number): void }) {
  ...
  const specialFeatures = configuration.specialFeatures.map((code) => SPECIAL_FEATURE_CATALOG.find((feature) => feature.id === code)?.label ?? code);
  ...
  return (
    <dl className={styles.reviewList}>
      ...
      <div><dt>วัสดุและส่วนพิเศษ</dt><dd>{materialQuality?.label ?? configuration.materialLevel} · {specialFeatures.join(", ") || "ไม่มี"}<br />บันทึกส่วนตัว: {configuration.privateNotes ? "มี (เก็บเฉพาะในเครื่อง)" : "ไม่มี"}<button onClick={() => onEdit(3)} type="button">แก้ไขวัสดุและส่วนพิเศษ</button></dd></div>
    </dl>
  );
}
