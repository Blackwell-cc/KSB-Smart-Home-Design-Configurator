  ...
  return (
    <div aria-label="ตัวอย่างหน้าจอวางแผนบ้าน" className={styles.previewCards} role="group">
      <div className={`${styles.cardFloat} ${styles.stylePosition}`}><article className={styles.previewCard}>
        ...
        <div className={styles.styleChoices}>{showcase.style.choices.map((choice, index) => <span className={styles.styleChoice} data-selected={index === 0} key={choice.label}><Image alt={`ตัวอย่างสไตล์ ${choice.label}`} fill sizes="92px" src="/concepts/contemporary-warm-luxury.png" />{index === 0 ? <i aria-hidden="true">✓</i> : null}</span>)}</div>
      </article></div>
